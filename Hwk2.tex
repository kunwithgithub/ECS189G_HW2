...
\tab{}In training this model, we have made some mistakes. The most dominate one is that we trained the model the same way as we trained the lm models. 
We reserved a test set with a random 5k objects and trained the models with the rest 95k objects. This attempt caused our H matrix contains many "NaN" rows because there might be some movies not covered in the 95k training set. These "NaN" rows made some of our predictions became "NaN" later when we tried to predict the test set rating from the A matrix, which made the MAPE() function really unhappy, and thus MAPE() gave us "NaN" result!
\\\\*
\tab{}But the strange thing was when we called the \$predict() directly from the trained model instead obtaining the predicting rating from the A matrix, the \$predict() function could always gave us values without "NaN" although the MAPE() results are undoubtly higher than the model trained with the full 100k objects.
\\\\*
